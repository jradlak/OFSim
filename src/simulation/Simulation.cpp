...
Simulation::Simulation() {
  //
  camera = std::make_unique<ofsim_renderer::Camera>(
      glm::vec3(-100.0, -160.0, 1000.0));
...
void Simulation::init() {
  ...
  rocket = std::make_unique<Rocket>("model3d_shader", rocketPos,
                                    rocket_initial_size);
  velocityIndicator = std::make_unique<VectorIndicator>(
      "model3d_shader", rocketPos, IndicatorType::VELOCITY);
  thrustIndicator = std::make_unique<VectorIndicator>(
      "model3d_shader", rocketPos, IndicatorType::THRUST_DIRECTION);

  // update camera position according to rocket:
  camera->position =
      rocket->properties().position + glm::dvec3(0.0, 0.024, 0.0);

  lag = 0;
  previous = currentTime();
  projection = glm::perspective((double)glm::radians(camera->Zoom),
                                (double)SCR_WIDTH / (double)SCR_HEIGHT, 0.001,
                                150000000.0);
}
...
void Simulation::physicsRocketInitialOrientation() {
  ...
  physics = std::make_unique<ofsim_math_and_physics::PhysicsSolver>(
      rocketProperties, CelestialBodyType::planet,
      SolarSystemConstants::earthSize, MS_PER_UPDATE);
  physics->establishInitialOrientation(rocketInitialPosition(theta, phi), theta,
                                       phi);
}
...
void Simulation::stop() {
  ...
  EventProcessor::getInstance()->povideRocketAndPhysics(rocket.get(),
                                                        physics.get());
  ...
  while (!mainWindow->shouldClose()) {
    ...
    if (simulationMode == SimulationMode::STANDARD_SIMULATION ||
      ...
      if (simulationMode == SimulationMode::PRESENTATION_MODE) {
        ...
        toTheMoon =
            SolarSystemConstants::moonPos - rocket->properties().position;
    ...
    rocket->renderParallelToVectorWithColorAndStretch(
        projection, view, SolarSystemConstants::lightPos, physics->thrustVector,
        vec3(0.55f), dvec3(1.0));
    ...
    if (simulationMode == SimulationMode::MANUAL_CONTROL ||
      ...
      velocityIndicator->renderWithMagnitudeAndDirection(
          projection, view, SolarSystemConstants::lightPos,
          rocket->properties().position, 1.0, velocityDirection);
      thrustIndicator->renderWithMagnitudeAndDirection(
          projection, view, SolarSystemConstants::lightPos,
          rocket->properties().position, physics->thrustMagnitude,
          thrustDirection);
    }

    if (simulationMode == SimulationMode::TRAJECTORY_PREDICTION ||
        simulationMode == SimulationMode::PRESENTATION_MODE) {
      trajectoryPrediction->render(projection, view,
                                   SolarSystemConstants::lightPos);

      if (simulationMode == SimulationMode::PRESENTATION_MODE) {
        double distance =
            glm::length(camera->position - rocket->properties().position);
...
void Simulation::renderHUD() {
  ...
  if (simulationMode == SimulationMode::DIAGNOSTICS_MODE) {
    ...
    gui->renderDiagnostics(diagnostics);
             ...
             simulationMode ==
                 SimulationMode::WAITING_FOR_BEGIN_MANUAL_CONTROL) {
    ...
    gui->renderCommandHistory(
        ofsim_events::EventProcessor::getInstance()->getCommandHistory());
    ...
    gui->plotTelemetry(
        ...
        telemetryCollector->atmPressureHistory,
        telemetryCollector->maxAtmPressure,
        telemetryCollector->accelerationHistory,
        telemetryCollector->maxAcceleration,
        telemetryCollector->minAcceleration);
    renderTelemetry(gui.get(), rocket.get(), physics->altitude, apogeum,
                    perygeum, physics->getAtmosphereDragForceMagnitude());
    ...
    gui->renderTranslationErrors(
        EventProcessor::getInstance()->getPythonError());
...
*/
void Simulation::userInteractionLogic(dvec3& toTheMoon, f64& radius,
                                      f64& step) {
  ...
  if (event.action == StateEvent::START_SIMULATION) {
    if (simulationMode == SimulationMode::WAITING_FOR_BEGIN) {
      ...
      if (this->orbitalProgramName.find(".py") != std::string::npos) {
        pythonMachine =
            std::make_unique<ofsim_python_integration::PythonMachine>();
        pythonThread = std::make_unique<std::thread>(
            &ofsim_python_integration::PythonMachine::runPythonOrbitalProgram,
            pythonMachine.get(), this->orbitalProgramSourceCode);
      } else if (this->orbitalProgramName.find(".oasm") != std::string::npos) {
        vmachine = std::make_unique<ofsim_vm::VMachine>(
            *EventProcessor::getInstance());
        vmachine->translateSourceCode(this->orbitalProgramSourceCode);
        vmThread = std::make_unique<std::thread>(&ofsim_vm::VMachine::start,
                                                 vmachine.get());
      } else {
  ...
  if (event.action == StateEvent::FILE_SAVE) {
    ofsim_infrastructure::saveSourceCode(SOURCE_CODE_FILE_NAME,
  ...
  if (event.action == StateEvent::CHANGE_MODE_TO_FROM_DIAGNOSTICS)  // o
  {
    if (simulationMode == SimulationMode::WAITING_FOR_BEGIN) {
      ...
      physics->establishInitialOrientation(rocketInitialPosition(theta, phi),
                                           theta, phi);
      RocketPhysicalProperties& rocketProperties = rocket->properties();
      rocketProperties.size *= 300000;
      camera->updatePosition(
          solarSystem->pointAboveEarthSurface(phi, theta - 20, 14421.0));
      ...
      event.action == StateEvent::CHANGE_MODE_TO_FROM_PREDICTION)  // m, k
  {
    ...
    if (simulationMode == SimulationMode::STANDARD_SIMULATION ||
      ...
      trajectoryPrediction->initWithPositions(
          physics->trajectoryPredictionX, physics->trajectoryPredictionY,
          physics->trajectoryPredictionZ, telemetryCollector->telemetryHistory);
      ...
      if (event.action == StateEvent::CHANGE_MODE_TO_FROM_PREDICTION)  // m
      {
        if (simulationMode != SimulationMode::TRAJECTORY_PREDICTION) {
          ...
          camera->updatePosition(
              solarSystem->pointAboveEarthSurface(pos[0], pos[1], pos[2]));
      ...
      if (event.action == StateEvent::CHANGE_MODE_TO_FORM_PRESENTATION)  // k
      {
        if (simulationMode != SimulationMode::PRESENTATION_MODE) {
          toTheMoon =
              rocket->properties().position - SolarSystemConstants::moonPos;
          radius = 0.000000001;
          step = 0.000000001;
          returnMode = simulationMode;
          simulationMode = SimulationMode::PRESENTATION_MODE;
...
void Simulation::renderTelemetry(ofsim_gui::Gui* gui, Rocket* rocket,
                                 double altitude, double apogee, double perigee,
                                 double atmosphereDragForceMagnitude) {
  TelemetryData data;
